// Package cases holds the case tables only MySQL expresses. They live next to
...
// InspectCases are the inspect results only MySQL syntax produces.
...
// SeeCases are the see cases only MySQL expresses.
func SeeCases() []testutil.SeeCase {
	return []testutil.SeeCase{
		...
		{Name: "a multi-table write over a derived table of visible columns", SQL: "DELETE c FROM contacts c JOIN (SELECT id FROM users) q ON q.id = c.id WHERE q.id > 0", Columns: []string{}},
		{
