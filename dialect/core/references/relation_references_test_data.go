...
func GetReferencesSelectTests(schema string) []ReferencesTest {
	return []ReferencesTest{
		...
		{
			// The parenthesized join arrives as an unaliased subquery around
			// t2, so walking the FROM clause for names does not reach t2 or t3.
			Name:          "parenthesized join",
			SQL:           "SELECT * FROM t1 JOIN (t2 JOIN t3 ON t2.c1 = t3.c31) ON t1.c1 = t2.c1",
			DefaultSchema: schema,
			ExpectedRefs: []RelationRef{
				{Schema: schema, Table: "t1", Alias: ""},
				{Schema: schema, Table: "t2", Alias: ""},
				{Schema: schema, Table: "t3", Alias: ""},
			},
		},
		{
		...
		{
			// The same aliasing one scope down. Nothing reaches a subquery
			// body unless the scope it belongs to asks the same question.
			Name:          "CTE referenced under an alias inside a subquery",
			SQL:           "WITH cte AS (SELECT c1 FROM t1) SELECT * FROM (SELECT * FROM cte tt1) s",
			DefaultSchema: schema,
			ExpectedRefs: []RelationRef{
				{Schema: schema, Table: "t1", Alias: ""},
				{Schema: "", Table: "s", Alias: ""},
				{Schema: "", Table: "cte", Alias: "tt1"},
			},
			ExpectedVtabs: []RelationRef{
				{Table: "cte", Columns: []Column{{Name: "c1", Type: "int", Nullable: false}}},
				{Table: "s", Columns: []Column{{Name: "c1", Type: "int", Nullable: false}}},
			},
		},
		{
			// A CTE reading another CTE under an alias. The alias is not a
			// relation of its own, and star expansion must not count the
			// columns once per name the source is known by.
			Name:          "CTE reading another CTE under an alias",
			SQL:           "WITH a AS (SELECT c1 FROM t1), b AS (SELECT * FROM a aa) SELECT * FROM b",
			DefaultSchema: schema,
			ExpectedRefs: []RelationRef{
				{Schema: schema, Table: "t1", Alias: ""},
				{Schema: "", Table: "b", Alias: ""},
			},
			ExpectedVtabs: []RelationRef{
				{Table: "a", Columns: []Column{{Name: "c1", Type: "int", Nullable: false}}},
				{Table: "b", Columns: []Column{{Name: "c1", Type: "int", Nullable: false}}},
			},
		},
		{
			...
			ExpectedRefs: []RelationRef{
				{Schema: schema, Table: "t1", Alias: ""},
				{Schema: schema, Table: "t2", Alias: ""},
				{Schema: "", Table: "used", Alias: ""},
			},
			// Both are still definitions, and both stay completable by name.
			ExpectedVtabs: []RelationRef{
				{Table: "used", Columns: []Column{{Name: "c1", Type: "int", Nullable: false}}},
				{Table: "unused", Columns: []Column{{Name: "c2", Type: "int", Nullable: false}}},
			},
		},
		{
...
// GetScopePositionTests pin where each relation's scope starts and ends. The
// positions are PostgreSQL's with the "public" schema.
func GetScopePositionTests() []ReferencesTest {
	return []ReferencesTest{
		{
			Name:         "simple table",
			SQL:          "SELECT * FROM t1",
			ExpectedRefs: []RelationRef{{Schema: "public", Table: "t1", ScopeStartPos: 0, ScopeEndPos: -1}},
		},
		{
			Name:         "table with alias",
			SQL:          "SELECT * FROM t1 tt1",
			ExpectedRefs: []RelationRef{{Schema: "public", Table: "t1", Alias: "tt1", ScopeStartPos: 0, ScopeEndPos: -1}},
		},
		{
			Name: "CTE",
			SQL:  "WITH cte AS (SELECT c1 FROM t1) SELECT * FROM cte",
