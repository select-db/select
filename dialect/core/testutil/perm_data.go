...
func permCases() []PermCase {
	return []PermCase{
		...
		},
		{
			...
			Denied: rowRights,
			Why:    "it reads a file the server can see and the caller may not, and the rows it read land in t1",
		},
		{
			On:     []string{"mysql"},
			Name:   "loading a file into named columns",
			SQL:    "LOAD DATA INFILE '/tmp/x.csv' INTO TABLE t1 (c1)",
			Needs:  []Right{Manage, mainT1(core.ActionInsert).Only("c1")},
			Denied: rowRights,
		...
		},
		{
			...
			Needs: []Right{
				Manage,
				mainT2(core.ActionInsert).Only("c3"),
				mainT1(core.ActionSelect).Only("c1"),
			},
			Denied: rowRights,
			Why:    "making the trigger is administration, and on every insert it copies the c1 of t1 into the c3 of t2",
		},
		{
			On:     []string{"sqlite"},
			Name:   "a trigger body carrying a write",
			SQL:    "CREATE TRIGGER tr AFTER INSERT ON t1 BEGIN UPDATE t2 SET c3 = 'x'; END",
			Needs:  []Right{Manage, mainT2(core.ActionUpdate).Only("c3")},
			Denied: rowRights,
			Why:    "the body rewrites c3 whenever anyone inserts into t1, and SQLite refuses a qualified name here, so this is the only spelling there is",
		},
		{
			On:     []string{"mysql"},
			Name:   "a trigger body carrying a write",
			SQL:    "CREATE TRIGGER tr AFTER INSERT ON t1 FOR EACH ROW BEGIN UPDATE t2 SET c3 = 'x'; END",
			Needs:  []Right{Manage, mainT2(core.ActionUpdate).Only("c3")},
			Denied: rowRights,
			Why:    "the body rewrites c3 whenever anyone inserts into t1, as it does on SQLite",
		},
		{
			On:     []string{"postgresql"},
			Name:   "a routine body carrying a write",
			SQL:    "CREATE PROCEDURE p9() LANGUAGE sql AS $$ DELETE FROM t1 WHERE c1 = 1 $$",
			Needs:  []Right{Manage, mainT1(core.ActionDelete), mainT1(core.ActionSelect).Only("c1")},
			Denied: rowRights,
		...
		},
		{
			...
			Denied: rowRights,
			Why:    "the element list holds the same view body the CREATE VIEW case is checked on",
		},
		{
			On:     []string{"postgresql"},
		...
		{
			On:     []string{"postgresql"},
			Name:   "an anonymous code block carrying a write",
		...
		},
		{
			...
			Needs:  []Right{Manage, mainT1(core.ActionInsert)},
			Denied: rowRights,
			Why:    "XML or CSV, the file is read for the server and the rows land in t1",
		},
		{
			On:     []string{"mysql"},
