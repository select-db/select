...
func permCases() []PermCase {
	return []PermCase{
		...
		},
		{
			...
			Needs: []Right{Manage},
			Op:    core.InspectOpInsert,
			Why:   "the parenthesis after a table name is a column list, not a call, and the name the metadata does not carry takes manage rather than insert on a schema nobody named",
		},
		{
			On:    []string{"postgresql"},
			...
			Needs: []Right{Manage},
			Op:    core.InspectOpUpdate,
			Why:   "a table may be named after a routine and still be a table, and one the metadata does not carry takes manage",
		},
		{
			On:    []string{"postgresql"},
		...
		// --- a name the metadata does not carry. It could be a table in any
		// schema, so no row right on the default one answers for it, and the
		// verb in front of it does not change that.
		{
			Name:  "reading a table the metadata does not carry",
			SQL:   "SELECT c1 FROM t9",
			Needs: []Right{Manage},
			Op:    core.InspectOpSelect,
			Why:   "a name nothing placed takes the right a statement nobody could read takes, not one naming an empty schema that no grant carries",
		},
		{
			Name:   "updating a table the metadata does not carry",
			SQL:    "UPDATE t9 SET c1 = 1",
			Needs:  []Right{Manage},
			Denied: rowRights,
			Op:     core.InspectOpUpdate,
			Why:    "a write of an unplaced name is the guess a read of it refuses to make, on the more destructive verb",
		},
		{
			Name:   "deleting from a table the metadata does not carry",
			SQL:    "DELETE FROM t9",
			Needs:  []Right{Manage},
			Denied: rowRights,
			Op:     core.InspectOpDelete,
		...
		{
			Name:   "inserting into a table the metadata does not carry",
		...
		{
			Name:  "reading a table the metadata does not carry, qualified",
			SQL:   "SELECT c1 FROM main.t9",
			Needs: []Right{{Action: core.ActionSelect, Schema: "main", Table: "t9"}},
			Op:    core.InspectOpSelect,
			Why:   "a qualified name says which schema to check it against, whether or not the metadata has caught up",
		},
		{
			Name:  "a scratch table read back in the same script",
			SQL:   "CREATE TABLE t9 (c1 integer); SELECT c1 FROM t9",
			Needs: []Right{Manage},
			Op:    core.InspectOpCreate,
			Why:   "creating the table takes manage, so the read of it in the next statement asks for nothing the script did not already need",
		},
