...
A row that does not open -- a column, an index, a trigger, a function, a setting -- shows its details on a single click instead: the same panel as **Infos...** on its right-click menu, with its type, its flags and where it sits.
