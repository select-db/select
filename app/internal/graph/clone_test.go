...
func fullTree() *WorkspaceNode {
	file := &FileNode{
		...
		Datasources:    []DatasourceRef{{ID: "db", Name: "warehouse"}},
	...
	item := &DatasourceItemNode{
		ID: "item", URI: "selectdb://i", Type: "schema", Name: "public", Path: "public",
		Badges: []string{"badge"}, Metadata: map[string]string{"k": "v"},
		ParentID: "db", Children: []*DatasourceItemNode{{ID: "child", Name: "orders"}},
	}
	db := &DatasourceNode{
		ID: "db", URI: "selectdb://db", Type: "datasource", Name: "warehouse",
		DBType: "postgresql", DSN: "postgres://", Proxified: true,
		SSH:         &DatasourceSSHConfig{Enabled: true, Host: "h", Port: 22, User: "u", AuthMethod: "agent", Password: "p", PrivateKey: "k"},
		FolderID:    "folder",
		WorkspaceID: "ws",
		Children:    []*DatasourceItemNode{item},
		Files:       []*FileNode{file},
...
func TestCloneCarriesEveryField(t *testing.T) {
	...
	requireNoZeroField(t, original.Datasources[0], "DatasourceNode")
	requireNoZeroField(t, original.Datasources[0].Children[0], "DatasourceItemNode")
...
func TestCloneOutlivesTheTree(t *testing.T) {
	...
	if len(cloned.Datasources) != 1 {
		t.Errorf("datasources: got %d, want 1", len(cloned.Datasources))
	}
