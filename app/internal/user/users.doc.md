# Users

Each workspace has its own set of users. Users are identified by their account (name, email, avatar) and can be members of multiple workspaces.

![The workspace's users: five people with their email, the roles each one holds, and the group they belong to. The first is marked Owner.](/shots/team.users.light.webp)

## Workspace owner

The user who creates a workspace is its **owner**. The owner has full access to all workspace settings and cannot be removed.

## Adding users

Users are added to a workspace by assigning them through the team management interface. Once added, a user can access the workspace and its databases according to the roles they are assigned.

## Roles

Users get permissions through **roles**. A user can have zero or more roles in a workspace, assigned directly or inherited from the [groups](/docs/workspace/groups/) they belong to. Permissions from all of those roles are combined. See [Roles](/docs/workspace/roles/), [Groups](/docs/workspace/groups/), and [Permissions](/docs/workspace/permissions/) for details.

> [!NOTE]
> A user with no roles has **no permissions** beyond basic read access to the workspace.
