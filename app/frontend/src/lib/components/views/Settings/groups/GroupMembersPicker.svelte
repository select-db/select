<script lang="ts">
	...
	let members = $state<group.GroupUserEntry[]>([]);
	...
	async function loadMembers() {
		const [r, err] = await tryCatch(ListGroupMembers, groupId);
		if (!err) members = (r as group.GroupUserEntry[]) ?? [];
	}
	...
	$effect(() => {
		if (members.length > 0 || wsUsers.length > 0) {
			loadAvatars([...members, ...wsUsers]);
		}
	});
	...
	let visibleUsers = $derived(members.slice(0, MAX_VISIBLE));
	let overflow = $derived(Math.max(0, members.length - MAX_VISIBLE));
	let assignedIds = $derived(new Set(members.map((u) => u.id)));

	async function toggleUser(userId: string) {
		if (assignedIds.has(userId)) {
			const match = members.find((m) => m.id === userId);
			if (!match) return;
			const [, err] = await tryCatch(RemoveUserFromGroup, match.user_to_group_id);
			if (err) {
				notify({ type: AlertType.Error, message: err?.message ?? 'Failed to remove user' });
				return;
			}
		} else {
			const [, err] = await tryCatch(AddUserToGroup, userId, groupId);
			if (err) {
				notify({ type: AlertType.Error, message: err?.message ?? 'Failed to add user' });
				return;
			}
		}
		await loadMembers();
...
<div class="group-members">
	...
	{:else}
		<button class="badge-strip" onclick={(e) => openMenu(e.currentTarget as HTMLElement)}>
			{#each visibleUsers as u (u.user_to_group_id)}
