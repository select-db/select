...
type DBInstanceNode = graph.DBInstanceNode;
...
export function getTableDDL(
	...
	const dbNode = graph.db_instances.find((d) => d.id === databaseId) as DBInstanceNode | undefined;
	if (!dbNode?.children) return null;

	const schemaNode = dbNode.children.find(
		(c) => c.type === 'schema' && (c.name ?? '') === schemaName
	) as DBInstanceItemNode | undefined;
	...
	const tableNode = (tablesGroup?.children ?? []).find((c) => (c.name ?? '') === tableName)
		?? (viewsGroup?.children ?? []).find((c) => (c.name ?? '') === tableName) as DBInstanceItemNode | undefined;
	...
	for (const idx of indexesGroup?.children ?? []) {
		const ddl = getMetadataSql(idx as DBInstanceItemNode);
	...
	for (const tr of triggersGroup?.children ?? []) {
		const ddl = getMetadataSql(tr as DBInstanceItemNode);
		if (ddl?.trim()) parts.push(ddl.trim());
	}
...
export function getSchemaTableOptionGroups(
	...
	const schemaNodes = dbNode.children.filter((c) => c.type === 'schema') as DBInstanceItemNode[];
