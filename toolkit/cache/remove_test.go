...
func TestOnRemoveFiresForEveryRemovalPath(t *testing.T) {
	...
	t.Run("DeleteFunc", func(t *testing.T) {
		...
		if len(got) != 2 {
			t.Fatalf("want 2 evictions, got %v", got)
		}
	})
	...
	t.Run("Set with an identical value does not evict", func(t *testing.T) {
		...
		c.Set("a", "same")
		if len(got) != 0 {
			t.Fatalf("want no eviction, got %v", got)
		}
	})
	...
	t.Run("TTL expiry via the background sweeper", func(t *testing.T) {
		...
		case k := <-fired:
			if k != "a" {
				t.Fatalf("evicted %q", k)
			}
		case <-time.After(2 * time.Second):
			t.Fatal("sweeper never evicted the expired entry")
		}
	})
}

// TestGetRefreshesTTL documents the property the pool-close grace period relies
// on: an actively used entry is never evicted out from under its users.
...
func TestOnRemoveConcurrent(t *testing.T) {
	...
	defer mu.Unlock()
	if count == 0 {
		t.Fatal("expected evictions under LRU pressure")
	}
}
