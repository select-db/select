...
// TestReadOnlyGatesAPINotSyncer pins the decoupling that a red CI once exposed:
// @app.api.readonly removes a column from the REST API write-set (IsWritable) but
...
// INSERT column list — binds NULL and violates the constraint. So a read-only
// column stays patchable (syncer writes and conflict-updates it) and exposed
// (readable/filterable), yet is not client-writable.
func TestReadOnlyGatesAPINotSyncer(t *testing.T) {
	tbl := RoleTable()
	// Add a NOT NULL @app.api.readonly column with a DB default, mirroring
	...
	tbl.Columns = append(tbl.Columns, RawColumn{
		...
		Default: "'local'::text", Comment: "Origin of the row. @app.api.readonly",
	})
	...
	if !source.ReadOnly {
		t.Error("source: @app.api.readonly not parsed")
	}
	...
	if !source.Patchable {
		t.Error("source: read-only must stay Patchable (syncer writes it)")
	}
	// Client can't forge it: excluded from the create/update body.
	if IsWritable(*source) {
		t.Error("source: read-only must not be API-writable")
	}
	// Still visible for read and $filter.
	if !source.Exposed {
		t.Error("source: read-only must stay Exposed (read/filter)")
	}
}
