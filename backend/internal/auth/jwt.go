...
// init loads RSA keys from file paths
var (
...
func loadKeys() {
	...
	privPem, err := os.ReadFile(privPath) // #nosec G703 -- path from operator-set PRIVATE_KEY_PATH env, not user input
	if err != nil {
		keyErr = fmt.Errorf("failed to read PRIVATE_KEY_PATH (%s): %v", privPath, err)
		return
	}
	privateKey, err = jwt.ParseRSAPrivateKeyFromPEM(privPem)
	if err != nil {
		keyErr = fmt.Errorf("failed to parse private key: %v", err)
		return
	}

	pubPem, err := os.ReadFile(pubPath) // #nosec G703 -- path from operator-set PUBLIC_KEY_PATH env, not user input
	if err != nil {
		keyErr = fmt.Errorf("failed to read PUBLIC_KEY_PATH (%s): %v", pubPath, err)
		return
	}
	publicKey, err = jwt.ParseRSAPublicKeyFromPEM(pubPem)
	if err != nil {
		keyErr = fmt.Errorf("failed to parse public key: %v", err)
		return
	}
}
