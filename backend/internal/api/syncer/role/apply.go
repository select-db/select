...
import (
	...
	"backend/internal/api/syncer/patch"
...
func Apply(ctx context.Context, userID string, c types.Commit, lastPulledAt time.Time) (bool, *types.RestoredItem, error) {
	...
	return patch.Apply(ctx, c, patch.Handler[generated.AppRole, generated.UpsertRoleParams]{
		...
		Merge: func(existing generated.AppRole, isNew bool, payload map[string]any) (generated.UpsertRoleParams, error) {
			// Fetch is by id only; reject hijacking a role that belongs to
			// another workspace (UpsertRole would rewrite its workspace_id).
