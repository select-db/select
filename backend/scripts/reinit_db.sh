...
# Requires: POSTGRES_DSN (Postgres URL) and psql.
#
# Usage (from backend/):
#   ./scripts/reinit_db.sh
#   # or: POSTGRES_DSN='postgres://user:pass@localhost:5432/dbname?sslmode=disable' ./scripts/reinit_db.sh
...
# Load POSTGRES_DSN from .env if present and POSTGRES_DSN not already set
if [[ -z "${POSTGRES_DSN:-}" ]] && [[ -f "$BACKEND_ROOT/.env" ]]; then
  set -a
...
if [[ -z "${POSTGRES_DSN:-}" ]]; then
  echo "Error: POSTGRES_DSN is not set. Set it in the environment or in $BACKEND_ROOT/.env" >&2
  exit 1
fi
...
psql "$POSTGRES_DSN" -v ON_ERROR_STOP=1 -c "TRUNCATE app.\"user\" CASCADE;"
